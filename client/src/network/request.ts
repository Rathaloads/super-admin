...
/** 请求拦截器：自动附加 JWT */
export const requestInterceptors = (config: InternalAxiosRequestConfig): InternalAxiosRequestConfig => {
...
export const requestErrorInterceptors = (error: AxiosError): Promise<AxiosError> => {
  return Promise.reject(error);
};

/** 响应拦截器：解包后端统一响应结构 */
export const responseInterceptors = (response: AxiosResponse<ApiResponse>) => {
...
export const responseErrorInterceptors = (error: AxiosError<ApiResponse>): Promise<never> => {
...
export const request = {
  ...
  setHeade: (header: Record<string, string>, reqInterceptors = requestInterceptors, resInterceptors = responseInterceptors) => {
    const cfg = { ...baseConfig };
    cfg.headers = {
      ...header,
    };
    const newInstance = createInstance(cfg);
    newInstance.interceptors.request.use(reqInterceptors, requestErrorInterceptors);
    newInstance.interceptors.response.use(resInterceptors, responseErrorInterceptors);
    return newInstance;
  },
};
